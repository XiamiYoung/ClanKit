...
-- Single-row table holding the full chat tree as JSON. Folders only exist
-- in the renderer's tree structure (not in chats.* columns), so the index
-- save/load path persists the entire tree blob here. Reconstructing the
-- tree from chats table alone is impossible — folder hierarchy is lost.
...
class ChatStore {
  ...
  /**
   * Get the chat tree (folders + chats nested) as it was last persisted by
   * the renderer. Returns null if no tree has been saved yet — callers
   * should fall back to the flat list from listChatIndex() in that case.
   */
  getChatTree() {
    const db = this._open()
    const row = db.prepare('SELECT tree_json FROM chat_tree WHERE id = 1').get()
    if (!row) return null
    try {
  ...
  /**
   * Persist the full chat tree blob. The renderer pushes the entire tree
   * (folders + chats) atomically; we don't try to diff because the tree is
   * cheap to serialize and frequent atomic snapshots are simpler than
   * incremental updates spread across multiple tables.
   */
  saveChatTree(tree) {
    if (!Array.isArray(tree)) return false
    const db = this._open()
    db.prepare(`
      INSERT INTO chat_tree (id, tree_json, updated_at) VALUES (1, ?, ?)
      ON CONFLICT(id) DO UPDATE SET tree_json = excluded.tree_json, updated_at = excluded.updated_at
