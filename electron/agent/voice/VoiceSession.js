...
class VoiceSession {
  ...
  async _runTurn(audioBuffer) {
    try {
      ...
      const tSTT = Date.now()
      logger.info(`[voice:timing] STT=${tSTT - t0}ms`)
      ...
      while ((m = re.exec(sentenceBuf)) !== null) {
        ...
        if (s) {
          if (!firstSentenceAt) {
            ...
            logger.info(`[voice:timing] LLM first-sentence=${firstSentenceAt - tLLMStart}ms (turn=${firstSentenceAt - (turnStart || tLLMStart)}ms)`)
          }
          this.onAiText(s)
        }
  ...
  async _greet() {
    // Skip the greeting if the user already started talking while we were
    // generating the chat digest (digest is an LLM call that can take 1–3s).
    // Without this guard, the greet LLM and the user-turn LLM run concurrently,
    // both speak via TTS, and the agent appears to "talk to itself".
    if (!this.active || this._voiceBusy || this.conversationHistory.length > 0) {
      if (this.active) this.onStatus('standby')
      return
    }
    ...
    const llmResult = await this.llmCall(messages, this.voiceConfig)
    // Re-check after the await: user may have started a turn during the greet
    // LLM round-trip. If so, drop the greeting silently.
    if (this._voiceBusy || this.conversationHistory.length > 0) {
      if (this.active) this.onStatus('standby')
      return
    }
...
${this.systemMemoryContent.trim()}`)
    ...
    // Prefer the LLM-generated identity card (~250 chars, fact-only). Fall back
    // to name + description when no card has been generated. Memory appended
    // separately. NEVER inject userAgent.systemPrompt (that is roleplay
    // instructions, not user data — would prime the agent to mimic the user).
    {
      const userLines = []
      const card = (this.userAgent.identityCard || '').trim()
      if (card) {
        userLines.push(card)
      } else {
...
This profile is a fact card, NOT a chat topic bank. Do NOT mine it for things to keep asking about — especially the user's job / industry / tech stack / projects. Do NOT free-associate from a job label like "software developer" / "teacher" / "doctor" into specific scenarios (AWS / Python / debug / students / patients) — those are NOT in the profile, you would be inventing them.`)
      }
    }
...
${this._chatDigest}`)
    ...
    // before VOICE CALL RULES so it sits in the highest-attention slot. Voice
    // replies are 1-3 sentences, so we keep this tight: just the rule + a
    // bad/good pair, no self-check ladder.
    if (this.userAgent && (this.userAgent.identityCard || this.userAgent.name || this.userAgent.description)) {
      parts.push(`## TOPIC GUARDRAIL
Do NOT use the user's job / industry / tech stack / projects from the profile above as a recurring topic to ask about. Do NOT close with "what about you, [user's work topic]?". When you ask a return question, ask about the **person** in front of you (mood, today, weather their side, what they ate, family / pet, what they're watching) — not their work / code / project / AWS / Python / Electron / debug. The fact that the user is e.g. a software developer is a fact, NOT a topic to keep chasing every reply.`)
    }
