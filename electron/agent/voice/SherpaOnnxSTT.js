...
// If a load attempt for a given modelDir failed, remember it so we don't keep
// re-triggering the same native crash on every audio chunk. Cleared by release().
let _failedModelDir = null
let _failedReason = null

function _classifyLoadError(rawMessage) {
  const msg = String(rawMessage || '')
  if (/API version|Ort|onnxruntime|opset/i.test(msg)) {
    return 'STT model is incompatible with the bundled ONNX runtime. The downloaded model requires a newer runtime. Please remove and re-download the voice model in Settings → Voice Call.'
  }
  return `Failed to load STT model: ${msg}`
}

function getRecognizer(modelDir) {
  if (_recognizer && _recognizerModelDir === modelDir) return _recognizer
  if (_failedModelDir === modelDir) {
    const err = new Error(_failedReason)
    err.code = 'STT_MODEL_BROKEN'
    throw err
  }
  ...
  try {
    ...
    _failedModelDir = modelDir
    _failedReason = _classifyLoadError(err?.message)
    logger.error(`[SherpaOnnxSTT] Recognizer load failed: ${err?.message || err}`)
    const wrapped = new Error(_failedReason)
    wrapped.code = 'STT_MODEL_BROKEN'
    throw wrapped
  }
}
...
class SherpaOnnxSTT {
  constructor({ modelDir }) {
    ...
    // If load fails (e.g. ORT/model version mismatch), surface the error to the
    // caller so the IPC layer can notify the user — don't silently continue.
  ...
  async transcribe(audioBuffer, mimeType = 'audio/wav', opts = {}) {
    ...
    try {
      ...
      logger.error(`[SherpaOnnxSTT] Decode failed: ${err?.message || err}`)
      throw new Error(`STT decode failed: ${err?.message || err}`)
    }
    const elapsed = Date.now() - t0

    logger.info(`[SherpaOnnxSTT] ${durationSecs.toFixed(1)}s audio → ${elapsed}ms`)
