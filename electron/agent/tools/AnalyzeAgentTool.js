...
class AnalyzeAgentTool extends BaseTool {
  ...
  constructor(memoryDir, targetAgentId, targetAgentName, targetAgentPrompt, dataPath, docPath, targetAgentType) {
    ...
    // User's preferred analysis-output language. Set by agentLoop from
    // config.language so the LLM's analysis labels (chemistry.type, keywords,
    // attachment.type, etc.) match the language of the report. When unset,
    // we fall back to auto-detection from chat content.
  ...
  async _extractSections(idx, onUpdate, forceRefresh = false) {
    ...
    if (!fs.existsSync(analysisCachePath)) {
      // No analyze_all cache yet — run it transparently rather than asking
      // the LLM to make a separate call. extract_sections is a one-shot
      // entry point: callers should be able to invoke it with no prior
      // setup and get sections back. Emits progress events from the inner
      // analyze run so the UI keeps streaming.
      emit('No prior analysis — running analyze_all first...')
      logger.agent('[AnalyzeAgentTool] extract_sections auto-running analyze_all', {
        agentId: this.targetAgentId,
      })
      const analyzeResult = await this._analyzeAll(idx, onUpdate, false)
      if (analyzeResult?.isError) {
        // Surface the analyze_all failure verbatim — the caller needs to know
        // the underlying problem (no LLM config, no messages, etc.), not a
        // generic "extract_sections failed".
        return analyzeResult
      }
      // After a successful analyze_all the cache file MUST exist; if it
      // somehow doesn't, fail loudly rather than entering an infinite loop.
      if (!fs.existsSync(analysisCachePath)) {
        return this._err('analyze_all completed but cache file is missing. This is a bug.')
      }
    }
    let cachedAnalysis
    try {
    ...
    const SECTION_GROUPS_OTHER = [
      ...
      { id: 'dialogue',    label: zh ? '对话与语录' : 'Dialogue & quotes',
        // 10-20 dialogue acts × ~150 chars + 16-20 key_quotes × ~80 chars
        // + 5-8 subtext rows easily exceeds 10K — under-budget caused the
        // LLM to truncate to 5 acts. 16K leaves headroom even on chats
        // with rich emotional history. Group split is preserved (still
        // one call) so cost only grows for this single bucket.
        keys: ['dialogue_theatre', 'key_quotes', 'subtext_decoder'], maxTokens: 16000 },
      { id: 'interaction', label: zh ? '互动模式'   : 'Interaction patterns',
        // interaction_tips upgraded to {tip, example, reason, icon} per item
        // (4-6 do + 3-5 dont) so it needs ~3x the previous budget.
        keys: ['interaction_tips', 'topic_preferences', 'signals', 'best_communication_times', 'expected_next'], maxTokens: 8000 },
    ...
    const SECTION_GROUPS_SELF = [
      ...
      { id: 'core',        label: zh ? '核心性格'   : 'Core personality',
        keys: ['persona_card', 'mbti', 'ocean', 'attachment'], maxTokens: 6144 },
      { id: 'dialogue',    label: zh ? '对话与语录' : 'Dialogue & quotes',
        // Same upgrade as system mode — see comment in SECTION_GROUPS_OTHER.
        keys: ['dialogue_theatre', 'key_quotes', 'subtext_decoder'], maxTokens: 16000 },
      { id: 'interaction', label: zh ? '话题与信号' : 'Topics & signals',
        keys: ['topic_preferences', 'signals', 'best_communication_times', 'expected_next'], maxTokens: 6000 },
  ...
  _getSectionSchemas(zh, isSelf = false) {
    ...
    "do": [
      {
        "tip": "<${zh ? '一句话做法标题' : 'one-line do title'}>",
        "example": "<${zh ? '一句具体例子,引用聊天里实际发生过的某次互动或描述具体场景,≤80 字' : 'concrete example referencing an actual past interaction or scene, ≤80 chars'}>",
        "reason": "<${zh ? '为什么这样做有效,基于性格特征 / 沟通模式,1 句话' : 'why this works, grounded in personality / communication pattern, one sentence'}>",
        "icon": "<${zh ? '一个真 emoji 字符' : 'a real Unicode emoji character'}>"
      }
      // 4-6 items
    ],
    "dont": [
      {
        "tip": "<${zh ? '一句话避雷标题' : 'one-line dont title'}>",
        "example": "<${zh ? '一句具体例子,描述会引发负面反应的场景,≤80 字' : 'concrete example showing the failure mode, ≤80 chars'}>",
        "reason": "<${zh ? '为什么会出问题,基于性格特征 / 触发点,1 句话' : 'why it fails, grounded in trigger / personality, one sentence'}>",
        "icon": "<${zh ? '一个真 emoji 字符' : 'a real Unicode emoji character'}>"
      }
      // 3-5 items
    ]
...
**⚠️ icon 字段规则(关键)**:所有 \`"icon"\` 字段**必须**是真实的 Unicode emoji 字符(如 🎯💬🔮😊❤️💡)。**绝对禁止**:
- 微信表情代码:\`[偷笑]\` \`[Chuckle]\` \`[捂脸]\` \`[Facepalm]\` \`[破涕为笑]\` \`[Lol]\` \`[Cry]\` \`[憨笑]\` 等——这些是平台特定的文本占位符,在 HTML 报告里会显示成字面文字,无法渲染
- 英文标签:\`smile\` \`laugh\` \`cry\` 等纯字母的"emoji 名"
- 数字 / ID:\`👍2\` \`🎯001\` 等

即使聊天样本里出现 \`[XXX]\` 形式的表情代码,也**不要**复制进 icon 字段。请凭语境选择合适的真实 emoji 字符。
...
**⚠️ Icon field rule (critical)**: every \`"icon"\` field **must** be a real Unicode emoji character (e.g. 🎯💬🔮😊❤️💡). **Strictly forbidden**:
- WeChat-style placeholder codes: \`[Chuckle]\` \`[偷笑]\` \`[Facepalm]\` \`[捂脸]\` \`[Lol]\` \`[Cry]\` \`[Sob]\` etc. — these are platform-specific text codes that render as literal brackets in the final HTML
- Bare emoji names: \`smile\` \`laugh\` \`cry\` and similar alphabetic "emoji name" strings
- Decorated codes: \`👍2\` \`🎯001\` etc.

Even if the chat samples contain \`[XXX]\`-style emoji codes, do NOT copy them into icon fields. Pick a real Unicode emoji that matches the meaning.
...
${messageBlock}`
  ...
  /**
   * Pin the analysis-output language. When set, this overrides chat-content
   * auto-detection — analysis labels and structured fields (chemistry.type,
   * keywords, attachment.type, etc.) will be emitted in this language.
   */
  setUserLanguage(lang) {
    const norm = String(lang || '').toLowerCase()
    if (norm.startsWith('zh')) this.userLanguage = 'zh'
    else if (norm.startsWith('en')) this.userLanguage = 'en'
    else this.userLanguage = null
  }

  /**
   * Resolve the analysis-output language. Priority:
   *   1. user-pinned (config.language) — what the user expects to read
   *   2. content-detected — fallback when no user pref exists
   * Reading user prose in Chinese while wanting an English report is a
   * legitimate use case (e.g. analyzing imported WeChat history for a
   * non-Chinese-speaking partner), so user pref MUST win.
   */
  _detectLanguage(messages) {
    if (this.userLanguage === 'zh' || this.userLanguage === 'en') return this.userLanguage
