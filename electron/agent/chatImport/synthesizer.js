...
async function synthesizePersonaSections(verified, profile, chatBlock, config, language) {
  ...
  //
  // Architectural rule (2026-05-03): only Identity / Honest Boundaries /
  // Relationship Timeline survive into memory. Mental Models / Decision
  // Heuristics / Values & Anti-Patterns / Relational Genealogy / Core
  // Tensions are abstractive analyses that already live in the persona
  // body's named chapters (思维内核 / 决策本能 / 你自己的功课 / 关系地图 /
  // 核心张力). Re-injecting them via memory caused the LLM to repeat
  // catchphrases on every reply. The verified-claim / tension / boundary
  // work is still done above (used by evidenceIndex), but only the three
  // episodic-relational sections round-trip through memory.
  const sections = {
    [getMemorySectionName('IDENTITY', language)]:              identityCard ? `\n${identityCard}\n` : '',
    [getMemorySectionName('HONEST_BOUNDARIES', language)]:     '\n' + (boundaries.length > 0 ? boundaries.map(b => `- ${b}`).join('\n') : '_(none derived yet)_') + '\n',
    [getMemorySectionName('RELATIONSHIP_TIMELINE', language)]: '\n' + (formatClaimList(grouped.relationshipTimeline) || '_(none extracted yet)_') + '\n',
  }
