...
function formatSpeechDnaBlock(speechDna) {
  if (!speechDna) return ''
  const lines = []
  lines.push('## SPEECH DNA — STYLE TENDENCIES')
  lines.push('Surface patterns extracted from real chat history. Use them when contextually natural; **do not force any single phrase, emoji, or ending into every reply**. The goal is to sound like this person on average across many turns, not to tic on every line.')
  lines.push('')

  const catchphrases = (speechDna.catchphrases || []).map(c => c.phrase || c).filter(Boolean)
  if (catchphrases.length > 0) {
    lines.push('Catchphrases (sprinkle sparingly — at most one every several turns; **never close every reply with the same phrase**):')
