...
## 决策本能
{6-8 条 IF→THEN 行为规则，从聊天里实际出现的反应模式提炼。**用抽象的行为类型描述（如：自嘲化解、追问细节、列证据反驳），不要引用具体口头禅或原话** —— 那是 ## 语言 DNA 章节负责的事：
- 被夸奖时 →
- 被质疑时 →
- 别人求助时 →
- 冷场时 →
...
## Decision Heuristics
{6-8 IF→THEN behavioral rules distilled from actual reactions in chat. **Use abstract behavior types (e.g. "deflect with self-mockery", "ask for concrete details", "cite evidence to push back") — do NOT quote specific catchphrases or verbatim words here**; that is the job of the ## Speech DNA section:
- When complimented →
- When challenged →
