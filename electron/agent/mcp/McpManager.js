...
class McpManager {
  ...
  async stopAll() {
    if (this._runtime) {
      // Cap close at 2.5s — if a child server hangs on shutdown we'd rather
      // orphan it than block app quit and stall an in-progress NSIS update.
      const close = this._runtime.close().catch(() => {})
      await Promise.race([
        close,
        new Promise(resolve => setTimeout(() => {
          logger.warn('[McpManager] runtime.close() exceeded 2.5s, forcing dispose')
          resolve()
        }, 2500)),
      ])
      this._runtime = null
