...
const BLOCK_COUNT_SPEC = {
  ...
  // ways_body / theatre_acts: render only what the LLM actually produced;
  // padding with placeholders makes the report look broken. We accept a
  // range so under-production gets a soft warning, not a hard failure.
  ways_body:           { className: 'way-row',           range: [3, 10] },
  theatre_acts:        { className: 'theatre-act',       range: [4, 10] },
  subtext_rows:        { className: 'subtext-row',       range: [1, 6] },
  ...
  do_items:            { className: 'apple-item',        range: [1, 4] },
  dont_items:          { className: 'apple-item',        range: [1, 4] },
  ...
  timeline_rows:       { className: 'tl-row',            range: [1, 6] },
  relation_modes:      { className: 'relation-row',      range: [1, 6] },
...
// WeChat-style emoji tags ([Chuckle], [偷笑], [捂脸], etc.) leak into the
// LLM-generated icon fields when the LLM picks an "emoji" verbatim out of
// imported chat history (where WeChat exports them as text codes). Browsers
// render them as literal text, breaking icon visibility. We strip these and
// either map well-known names to a real Unicode emoji, or fall back to the
// caller-supplied default.
const WECHAT_EMOJI_MAP = {
  // Most common tags from WeChat exports — both EN and ZH names
  'chuckle':         '😏', '偷笑':       '😏', 'lol':       '😏',
  'facepalm':        '🤦', '捂脸':       '🤦',
  '破涕为笑':         '🤣', 'rofl':       '🤣',
  'grin':            '😄', '憨笑':       '😄',
...
function normalizeIcon(raw, fallback = '✨') {
  ...
  // WeChat-style [Name] markup — strip and look up
  const m = s.match(/^\[([^\]]+)\]$/)
  if (m) {
    const key = m[1].trim().toLowerCase()
    if (WECHAT_EMOJI_MAP[key]) return WECHAT_EMOJI_MAP[key]
    // Unknown WeChat tag — drop it; render-time fallback beats literal text
    return fallback
  }
  // Plain alphanumeric word (no emoji codepoints) — also looks like a stray
  // text code, fall back. Heuristic: if the entire string is ASCII letters
  // and digits with no emoji-range chars, it's not an icon.
...
// interaction_tips.do/dont accept TWO shapes:
//   - legacy: ["string", ...]
//   - rich:   [{tip, example, reason, icon, title}, ...]
// Many builders treated rich items as strings, producing literal "[object Object]"
// in the rendered HTML. tipText extracts the user-visible label from either shape.
function tipText(item) {
  if (item == null) return ''
  if (typeof item === 'string') return item
  if (typeof item === 'object') return String(item.tip || item.title || item.text || '').trim()
  return String(item)
}
function tipExample(item) {
  if (item && typeof item === 'object') return String(item.example || '').trim()
  return ''
}
function tipReason(item) {
  if (item && typeof item === 'object') return String(item.reason || '').trim()
  return ''
}
...
function buildDScalars(sections, stats, agentName, lang, isSelf, L) {
  ...
  const peakYearEntry = Object.entries(yearly).sort((a, b) => b[1] - a[1])[0] || ['', 0]
  const peakMonthEntry = Object.entries(monthly).sort((a, b) => b[1] - a[1])[0] || ['', 0]
  ...
  // Year chart granularity: switch to monthly buckets when all chat history
  // falls within one calendar year. A single yearly point is a degenerate
  // line (already handled by template fallback) but per-month buckets give
  // the user a real trend curve. The `year` field is kept as the bucket
  // label to avoid touching the template — for monthly mode it carries
  // "YYYY-MM" instead of just "YYYY".
  const monthEntriesAll = Object.entries(monthly).sort(([a], [b]) => a.localeCompare(b))
  const useMonthlyTrend = yearsCount <= 1 && monthEntriesAll.length >= 2
  const yearChartData = useMonthlyTrend
    ? monthEntriesAll.map(([ym, t]) => {
        const perSender = monthlyBySender[ym]
        if (perSender && himKey) {
          const h = perSender[himKey] || 0
          let m = 0
          for (const [k, v] of Object.entries(perSender)) if (k !== himKey) m += v
          return { year: ym, him: h, me: m }
        }
        const h = Math.round(t * pctHim / 100)
        return { year: ym, him: h, me: t - h }
      })
  ...
  return {
    ...
    // years_count carries the trend chart's primary-axis count: number of
    // years when we have multi-year data, number of months when the chart
    // switched to monthly granularity. The string template uses it inside
    // a unit-aware label (see trend_unit_label below) so "1 years" never
    // shows up in the rendered HTML.
    years_count: String(useMonthlyTrend ? monthEntriesAll.length : yearsCount),
    trend_unit_label: useMonthlyTrend
      ? (lang === 'zh' ? '个月' : (monthEntriesAll.length === 1 ? 'month' : 'months'))
      : (lang === 'zh' ? '年' : (yearsCount === 1 ? 'year' : 'years')),
    // "Annual" vs "Monthly" — drives Section 02 title + chart-card title so
    // a single-year chart isn't labeled "Annual" (misleading) when its X axis
    // is months.
    trend_period_label: useMonthlyTrend
      ? (lang === 'zh' ? '月度' : 'Monthly')
      : (lang === 'zh' ? '年度' : 'Annual'),
    // Comparison wording adapts the same way — monthly mode says
    // "month-over-month", yearly mode says "year-over-year".
    trend_compare_label: useMonthlyTrend
      ? (lang === 'zh' ? '逐月' : 'Month-over-month')
      : (lang === 'zh' ? '逐年' : 'Year-over-year'),
    date_first: String(dateFirst).substring(0, 4) || '?',
    date_last: String(dateLast).substring(0, 4) || '?',
    // Peak label adapts to chart granularity — monthly mode → "YYYY-MM",
    // yearly mode → "YYYY". Same field name keeps strings.json untouched.
    peak_year: String(useMonthlyTrend ? peakMonthEntry[0] : peakYearEntry[0]),
    peak_count_fmt: fmt(useMonthlyTrend ? peakMonthEntry[1] : peakYearEntry[1]),
    ...
    ...(() => {
      ...
      const icon = normalizeIcon(verdict.icon, tier.icon)
    ...
    // Chart-label i18n. Previously these strings were hardcoded Chinese in
    // template.html, which leaked Chinese into English reports. Pass them
    // through D scalars so the language switch is real, not partial.
    peak_label_text: lang === 'zh' ? '峰值' : 'Peak',
    weekday_labels_json: JSON.stringify(
      lang === 'zh'
        ? ['周一', '周二', '周三', '周四', '周五', '周六', '周日']
        : ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']
    ),
    month_short_suffix: lang === 'zh' ? '月' : '',
    heatmap_intensity_label: lang === 'zh' ? '热度' : 'Activity',
  }
}
...
function buildDeepAnalysis(sections, lang) {
  ...
  const sources = [
    ...
    () => {
      ...
      const doSeed = (tips.do || []).slice(0, 2).map(t => esc(tipText(t))).filter(Boolean).join(sep)
      return [lovesText ? esc(lovesText) : '', doSeed].filter(Boolean).join(sep)
    },
    // 13. Defense mechanisms — tips.dont (ONLY place for dont)
    () => (tips.dont || []).slice(0, 4).map(t => esc(tipText(t))).filter(Boolean).join(sep),
...
function buildTheatreActs(sections, lang) {
  ...
  // Render only the acts the LLM actually produced — never pad with empty
  // "Scene pending" placeholders. Empty placeholders are worse UX than
  // showing fewer acts: they make the report look broken even when the
  // real ones are good. Min/max bounds: cap at 10, but allow as few as 1.
  const acts = theatre.slice(0, 10).filter(a => {
    if (!a) return false
    const hasTitle = a.scene_title && String(a.scene_title).trim()
    const hasMessages = Array.isArray(a.messages) && a.messages.length > 0
    const hasAnalysis = a.analysis && String(a.analysis).trim()
    // Require at least one of: a real title, real messages, or real analysis.
    // Drops LLM-emitted skeleton entries with all-empty fields.
    return hasTitle || hasMessages || hasAnalysis
  })
  if (acts.length === 0) {
    return `<div class="theatre-act" style="opacity:0.6;"><div class="theatre-meta"><div class="theatre-num">—</div></div><div><div class="theatre-scene">${lang === 'zh' ? '暂无可展示的对话场景。' : 'No dialogue acts available.'}</div></div></div>`
  }
...
function buildSubtextRows(sections, lang) {
  ...
  const isSelf = sections._isSelf === true
  // The Subtext Decoder section explains what the SUBJECT (the partner for an
  // "other" report, the user for a "self" report) said vs. meant. When the LLM
  // attaches an explicit `speaker` field, we filter to entries that match the
  // subject so we don't surface user quotes inside a partner-analysis report
  // (and vice-versa).
  const subjectSpeakers = isSelf
    ? new Set(['me', 'self', 'user', '我', '本人'])
    : new Set(['them', 'partner', 'subject', 'agent', 'him', 'her', '对方', 'they'])
  const filtered = sub.filter(item => {
    if (!item || typeof item !== 'object') return false
    const sp = String(item.speaker || '').trim().toLowerCase()
    if (!sp) return true  // no speaker tag → trust LLM, render anyway
    if (subjectSpeakers.has(sp)) return true
    // Explicit other-side speaker → drop
    return false
  })
  const real = filtered.slice(0, 6).filter(item =>
    item && (String(item.surface || '').trim() || String(item.decoded || '').trim())
  )
  if (real.length === 0) {
    const empty = lang === 'zh' ? '尚未提取到有代表性的潜台词样本' : 'No representative subtext samples extracted yet'
    return `<div class="subtext-row" style="grid-column:1/-1;color:#9ca3af;text-align:center;padding:16px 0;">${empty}</div>`
  }
  return real.map(item => {
    const speakerTag = item.speaker
      ? `<span class="subtext-speaker" style="font-size:11px;color:#9ca3af;margin-right:6px;">[${esc(item.speaker)}]</span>`
      : ''
    return `<div class="subtext-row">\n  <div class="subtext-surface">${speakerTag}"${esc(item.surface || '')}"</div>\n  <div class="subtext-decoded"><span class="arrow">→</span>${esc(item.decoded || '')}</div>\n  <div class="subtext-evidence">${esc(item.evidence || '')}</div>\n</div>`
  }).join('\n')
}
...
function buildModeRows(sections, lang) {
  ...
  const descs = [
    ...
    tipText((tips.do || [])[0]) || (lang === 'zh' ? '用具体行动代替承诺' : 'Actions over promises'),
    tipText((tips.dont || [])[0]) || (lang === 'zh' ? '对不合理要求明确拒绝' : 'Clear refusal of unreasonable demands'),
  ]
...
function buildRelationModes(sections, lang) {
  ...
  for (const item of (tips.do || []).slice(0, 3)) {
    const label = tipText(item)
    if (!label) continue
    rows.push({ ico: normalizeIcon(item && item.icon, '⚙️'), label, desc: tipExample(item), score: lang === 'zh' ? '核心' : 'Core' })
  }
  for (const item of (tips.dont || []).slice(0, 2)) {
    const label = tipText(item)
    if (!label) continue
    rows.push({ ico: normalizeIcon(item && item.icon, '⚠️'), label, desc: tipExample(item), score: lang === 'zh' ? '注意' : 'Note' })
  }
  if (rows.length === 0) {
    return `<div class="relation-row"><span class="relation-ico">💡</span><div class="relation-content"><div class="relation-label">${lang === 'zh' ? '互动模式待积累更多数据后呈现' : 'Patterns will emerge with more data'}</div><div class="relation-desc"></div></div><span class="relation-score">—</span></div>`
  }
  return rows.slice(0, 6).map(r =>
    `<div class="relation-row">\n  <span class="relation-ico">${r.ico}</span>\n  <div class="relation-content">\n    <div class="relation-label">${esc(r.label)}</div>\n    <div class="relation-desc">${esc(r.desc || '')}</div>\n  </div>\n  <span class="relation-score">${esc(r.score)}</span>\n</div>`
  ).join('\n')
}
...
function buildTimelineRows(sections, stats, lang) {
  ...
  // Render only periods we actually have. Padding with "??" / "—" placeholder
  // rows misrepresents data range and looks like a render bug. When the chat
  // history spans only one year, show a single Initial-stage row.
  const real = periods.slice(0, 6)
  if (real.length === 0) {
    const empty = lang === 'zh' ? '需要更多数据来识别关系阶段' : 'Need more data to identify stages'
    return `<div class="tl-row"><div class="tl-dot" style="background:${TL_COLORS[0]};">·</div><div><div class="tl-label">${empty}</div><div class="tl-desc"></div></div></div>`
  }
  return real.map((r, i) =>
    `<div class="tl-row">\n  <div class="tl-dot" style="background:${TL_COLORS[i % TL_COLORS.length]};">${esc(r.dot)}</div>\n  <div>\n    <div class="tl-label">${esc(r.label)}<span class="tl-year">· ${esc(r.year)}</span></div>\n    <div class="tl-desc">${esc(r.desc)}</div>\n  </div>\n</div>`
  ).join('\n')
}

// ─── Do / Don't items — 4 each ───

function buildAppleItems(arr, lang) {
  // Normalize each entry through tipText so rich-object items (post-2026-05
  // schema: {tip, example, reason, icon}) and legacy strings both render.
  const normalized = (arr || []).map(tipText).filter(s => s && s.trim())
  if (normalized.length === 0) {
    const empty = lang === 'zh' ? '尚未识别建议项' : 'No items yet'
    return `<div class="apple-item" style="color:#9ca3af;">${empty}</div>`
  }
  // Render up to 4 real items; render fewer rather than padding with "—".
  return normalized.slice(0, 4).map(t => `<div class="apple-item">${esc(t)}</div>`).join('\n')
}
...
function buildWaysBody(sections, lang) {
  ...
  // Backwards-compat with both interaction_tips.do shapes:
  //   - legacy: ["string", "string"]                   (one-line tip only)
  //   - rich:   [{tip, example, reason, icon}]         (post-2026-05 schema)
  // Old caches have the legacy shape — keep them readable but we no longer
  // emit a hard-coded "From interaction tips" reason; if the LLM only gave
  // a string we render it WITHOUT the example/reason rows so the card is
  // visually cleaner instead of misleading.
  for (const item of (tips.do || [])) {
    if (src.length >= 10) break
    if (typeof item === 'string') {
      src.push({ title: item, example: '', icon: '💡', reason: '' })
  ...
  // Render only items with at least a real title — DO NOT pad with "(待补充)"
  // placeholders. A short list with real content beats a padded list with
  // fake content.
  const ways = src.filter(w => w.title && String(w.title).trim()).slice(0, 10)
  if (ways.length === 0) {
    return `<div class="way-row" style="opacity:0.6;"><div class="way-num-col"><div class="way-num">—</div></div><div><div class="way-title">${lang === 'zh' ? '暂无可展示的互动建议。' : 'No interaction tips available.'}</div></div></div>`
  }
  ...
  return ways.map((w, i) => {
    ...
    // Skip the example / reason boxes entirely when their content is empty
    // — showing labelled empty boxes with no body looked like a UI bug.
    const exampleBlock = w.example && String(w.example).trim()
      ? `\n    <div class="way-example" style="border-left:3px solid ${color};">\n      <div class="way-kicker">${exLabel}</div>\n      <div>${esc(w.example)}</div>\n    </div>`
      : ''
    const reasonBlock = w.reason && String(w.reason).trim()
      ? `\n    <div class="way-reason">\n      <div class="way-kicker" style="color:${color};">${whyLabel}</div>\n      <div class="way-reason-body">${esc(w.reason)}</div>\n    </div>`
      : ''
    return `<div class="way-row">\n  <div class="way-num-col">\n    <div class="way-num" style="color:${color};">${num}</div>\n    <div class="way-icon">${normalizeIcon(w.icon, '🎯')}</div>\n  </div>\n  <div>\n    <div class="way-title">${esc(w.title)}</div>${exampleBlock}${reasonBlock}\n  </div>\n</div>`
  }).join('\n')
}
...
function buildAllHtml(sections, stats, lang) {
  ...
  return {
    ...
    do_items:            buildAppleItems(tips.do, lang),
    dont_items:          buildAppleItems(tips.dont, lang),
...
function buildUniqueMemories(sections, lang) {
  ...
  <div class="mem-head">
    ...
    <div class="mem-icon" style="color:${color};">${normalizeIcon(m.icon, '✨')}</div>
...
function buildLifeTimeline(sections, lang) {
  ...
  return events.slice(0, 20).map(e => {
    ...
    return `<div class="tl-row life-row">
      ...
      <div class="tl-body">
        <div class="tl-head">
          <span class="tl-icon">${normalizeIcon(e.icon, '•')}</span>
...
class RenderReportTool extends BaseTool {
  constructor(context = {}) {
    ...
    // Optional: analyze tool reference, populated only in analysis chats so we
    // can auto-run extract_sections when the sections cache is missing,
    // instead of forcing the LLM to make a separate call.
    this.analysisTool = context.analysisTool || null
  }
  ...
  async execute(toolCallId, params, signal, onUpdate) {
    try {
      ...
      if (realKeys.length < 5 || selfKeysLooseCheck) {
        ...
        // Still not enough data? Try to auto-run extract_sections (which
        // itself transparently runs analyze_all when needed) so the LLM
        // doesn't have to make a separate call to fix this. Only attempts
        // when we have an analysisTool reference (analysis chats).
        const afterKeys = Object.keys(sections).filter(k => !k.startsWith('_'))
        if (afterKeys.length < 5) {
          if (this.analysisTool && typeof this.analysisTool.execute === 'function') {
            logger.agent('[RenderReportTool] sections cache missing — auto-running extract_sections')
            try {
              const onUpdate = (payload) => { /* progress events surface via the analysis tool's own onUpdate path */ void payload }
              const extractResult = await this.analysisTool.execute(
                `auto_extract_${Date.now()}`,
              ...
              // After extract_sections succeeds the cache file exists; reload.
              const reloadPath = this._findSectionsCache()
              if (reloadPath) {
                const cached = JSON.parse(fs.readFileSync(reloadPath, 'utf8'))
                if (cached.sections) sections = cached.sections
                if (Object.keys(stats).length === 0 && cached.stats) stats = cached.stats
                loadedFromCache = true
                logger.agent('[RenderReportTool] auto-extracted sections', {
                  keys: Object.keys(sections).length,
                })
              }
            } catch (err) {
              logger.warn('[RenderReportTool] auto extract_sections failed', err.message)
            }
          }

          // After auto-extract attempt, check again. If still empty, give up
          // with a clear instruction.
          const finalKeys = Object.keys(sections).filter(k => !k.startsWith('_'))
          if (finalKeys.length < 5) {
            return this._err(
              'No sections data available. Run analyze_agent_history({action: "extract_sections"}) ' +
              'first — it will automatically run analyze_all if needed, then return the sections data ' +
              'you can pass into this tool.'
            )
          }
        }
      }
      ...
      // 1..N sequence that matches the visual position and TOC entries.
      {
        ...
        // - "star" entries (Verdict ★, Self·N) are sticky badges that must
        //   NOT consume a regular section index. Increment ONLY for normal
        //   entries — otherwise the first numbered item gets bumped to "02"
        //   and the sequence skips "01" (visible bug to users).
        // - Verdict ★ is rebadged "00" so the TOC reads as a true Section 0
        //   instead of a floating star outside the numbering.
        let tocIdx = 0
        out = out.replace(
          /(<a class="toc-item" href="(#[^"]*)"[^>]*><span class="toc-num( star)?">)([^<]*)(<\/span>)/g,
          (_m, pre, href, starFlag, label, post) => {
            const isStar = !!starFlag
            if (isStar) {
              // Verdict star → "00"; preserve any pre-formatted star labels (e.g. "S·1").
              if (href === '#verdict') return `${pre}00${post}`
