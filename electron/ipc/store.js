...
function register({ DEFAULT_CONFIG }) {
  ...
  ipcMain.handle('store:get-chat-index', async () => {
    ...
    // Prefer the full tree (folders + nested chats) the renderer last
    // persisted. Fall back to a flat chat list when no tree exists yet
    // (fresh install before any folder was created, or pre-tree-blob users).
    // Reconcile: if any chats exist in the chats table that the tree does
    // NOT mention (e.g. chat created via a path that bypassed persistIndex),
    // append them at root so they're not invisible.
    const tree = store.getChatTree()
    const flat = store.listChatIndex()
    if (!tree) return flat
    const seen = new Set()
    const collect = (nodes) => {
    ...
    collect(tree)
    const orphans = flat.filter(c => !seen.has(c.id)).map(c => ({ type: 'chat', ...c }))
    return orphans.length ? [...orphans, ...tree] : tree
  })

  ipcMain.handle('store:save-chat-index', async (_, index) => {
    // Persist the full tree blob. Folders only exist in this tree (not in
    // the chats table), so we must store the renderer's payload — the
    // earlier no-op handler silently dropped folder hierarchy on every
    // restart, making "create folder, restart, folder gone" a routine bug.
    if (!Array.isArray(index)) return false
    return _chatStore().saveChatTree(index)
  })
  ...
  ipcMain.handle('store:save-agents', async (_, data) => {
    ...
    // Capture user-persona prompts BEFORE replaceKind so we can detect which
    // personas actually changed prompt — those are the ones whose identity
    // card needs regenerating. Pre-existing cards are preserved by
    // replaceKind itself; we only spend LLM tokens when prompt content
    // actually changed.
    const oldUserPrompts = new Map(
      store.getByKind('user').map(u => [u.id, u.prompt || ''])
    )
    ...
    // Regenerate identity cards for user personas whose prompt changed (or
    // are brand-new with no existing card). Synchronous: a single save
    // typically touches 1 persona, so the latency is one LLM call (~1-3s).
    // Multi-edit saves do them sequentially — same total time as the user
    // expects. Failures fall back to the minimal card inside the builder.
    try {
      const { buildUserIdentityCard } = require('../agent/userIdentityCardBuilder')
      const cfg = ds.readJSON(p().CONFIG_FILE, {})
      const language = cfg?.language === 'zh' ? 'zh' : 'en'
      const llmConfig = {
        ...cfg,
        utilityModel: cfg?.utilityModel || null,
      }
      const incoming = data?.personas?.items || []
      for (const u of incoming) {
        if (!u?.id) continue
        const oldPrompt = oldUserPrompts.get(u.id) || ''
        const newPrompt = u.prompt || ''
        const promptChanged = oldPrompt !== newPrompt
        const hasCard = store.getById(u.id)?.identityCard
        if (!promptChanged && hasCard) continue
        // Generate (or skip silently if no utility model — builder returns
        // a minimal card in that case).
        const card = await buildUserIdentityCard(u, llmConfig, language)
        if (card) store.setIdentityCard(u.id, card)
      }
    } catch (err) {
      logger.warn('store:save-agents identity-card regen failed:', err.message)
    }
