...
const weixinVault = require('../agent/chatImport/weixinVault')
...
ipcMain.handle('agent:import-decrypt-wechat', async (event, { dbDir } = {}) => {
  try {
    ...
    const result = weixinVault.decrypt({
      output: decryptedDir,
...
// ─── agent:regenerate-user-identity-card ──────────────────────────────────
// Manual regen entry point — fires when the user clicks "Regenerate" on a
// user-persona editor. Idempotent. Returns the new card text on success so
// the UI can preview it inline without reloading.

ipcMain.handle('agent:regenerate-user-identity-card', async (_event, { agentId }) => {
  try {
    ...
    if (agent.type !== 'user') return { success: false, error: 'identity card only applies to user-type agents' }
    const cfg = ds.readJSON(ds.paths().CONFIG_FILE, {})
    const language = cfg?.language === 'zh' ? 'zh' : 'en'
    const llmConfig = { ...cfg, utilityModel: cfg?.utilityModel || null }
    const { buildUserIdentityCard } = require('../agent/userIdentityCardBuilder')
    const card = await buildUserIdentityCard(agent, llmConfig, language)
    if (!card) return { success: false, error: 'card generation returned empty' }
    store.setIdentityCard(agentId, card)
    logger.info(`agent:regenerate-user-identity-card: regenerated for ${agentId} (${card.length} chars)`)
    return { success: true, card }
  } catch (err) {
    logger.error('agent:regenerate-user-identity-card error', err.message)
    return { success: false, error: err.message }
  }
})
...
async function _extractWeChat(event, contactName, dbDir) {
  ...
  if (!resolvedDbDir) {
    ...
    sendProgress(event, { step: 'key', progress: 5, message: 'Starting WeChat decryption...' })
    const decryptResult = weixinVault.decrypt({
      output: decryptedDir,
      emitFn: (step, progress, message) => sendProgress(event, { step, progress, message }),
    })
