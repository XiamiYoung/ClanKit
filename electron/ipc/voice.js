  ...
  ipcMain.handle('voice:start', async (event, params) => {
    ...
    try {
      ...
      const llmCall = async (messages, vc, opts = {}) => {
        ...
        const { resolveProviderCreds, isOpenAICompatibleProvider } = require('./agentRuntimeUtils')
        ...
        const isOpenAI = isOpenAICompatibleProvider(resolvedType)
        const clientCfg = {
          apiKey: creds.apiKey,
        ...
        if (isGoogle) {
          ...
          // Anthropic — uses AnthropicClient. (OpenRouter is OpenAI-compatible, handled above.)
      ...
      // Notify renderer so the in-call error UI shows the message (the return
      // value alone is not enough — listeners expect 'voice:error' events too).
      const mainWindow = winRef.get()
      if (mainWindow && !mainWindow.isDestroyed()) {
        mainWindow.webContents.send('voice:error', { message: err.message, code: err.code || null })
      }
      // Tear down any half-initialised session so the next start attempt is clean.
      if (activeVoiceSession) {
        try { activeVoiceSession.stop() } catch {}
        activeVoiceSession = null
      }
