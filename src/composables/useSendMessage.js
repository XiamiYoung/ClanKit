...
export function useSendMessage({
  ...
  async function sendMessage() {
    ...
    // Guard: refuse to send if the chat references any deleted agent.
    // Per policy 2026-05-03 we do NOT silently substitute the default agent
    // — the chat must surface a clear error and the user must reassign or
    // delete the chat. Checks systemAgentId, groupAgentIds, and userAgentId.
    {
      const c = chatsStore.chats.find(ch => ch.id === chatId)
      const refsToCheck = []
      if (c?.systemAgentId) refsToCheck.push(c.systemAgentId)
      if (Array.isArray(c?.groupAgentIds)) refsToCheck.push(...c.groupAgentIds)
      if (c?.userAgentId) refsToCheck.push(c.userAgentId)
      const deletedRef = refsToCheck.find(id => agentsStore.isAgentDeleted(id))
      if (deletedRef) {
        dbg(`sendMessage refused: chat references deleted agent ${deletedRef}`, 'error')
