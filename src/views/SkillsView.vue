<template>
  <div class="h-full flex flex-col overflow-hidden" style="background:#F8FAFC;">
    ...
    <template v-if="!selectedSkill">
      ...
      <div class="catalog-header">
        <div style="display:flex; align-items:center; justify-content:space-between;">
          <div>
            <h1 style="font-family:'Figtree',sans-serif; font-size:var(--fs-page-title); font-weight:700; color:#0F172A; margin:0;">Skills</h1>
            <p style="font-family:'Noto Sans',sans-serif; font-size:var(--fs-body); color:#475569; margin:4px 0 0 0;">
      ...
      <div v-if="skillsStore.loading" class="flex-1 flex items-center justify-center">
        <p style="font-family:'Noto Sans',sans-serif; font-size:var(--fs-body); color:#64748B;">Loading skills...</p>
      </div>

      <!-- Error -->
      <div v-else-if="skillsStore.error" class="flex-1 flex items-center justify-center">
        <div class="text-center">
          <p style="font-family:'Noto Sans',sans-serif; font-size:var(--fs-body); color:#dc2626;">{{ skillsStore.error }}</p>
          <button
            @click="refresh"
            class="mt-3 px-4 py-2 rounded-lg cursor-pointer"
            style="background:#3B82F6; color:#fff; border:none; font-family:'Noto Sans',sans-serif;"
          >Retry</button>
        </div>
      </div>
      ...
      <div v-else-if="skillsStore.skills.length === 0" class="flex-1 flex items-center justify-center">
        <div class="text-center" style="max-width:420px;">
          ...
          <p style="font-family:'Noto Sans',sans-serif; font-size:var(--fs-body); color:#64748B; line-height:1.6; margin:0 0 16px;">
            Create skill folders in your skills directory. Each folder should contain a
            <code style="background:#F1F5F9; padding:2px 6px; border-radius:4px; font-size:0.875em;">SKILL.md</code>
            file with the skill's system prompt.
          </p>
          <p style="font-family:'Noto Sans',sans-serif; font-size:var(--fs-secondary); color:#94A3B8;">
            Default path: <code style="background:#F1F5F9; padding:2px 6px; border-radius:4px; font-size:0.875em;">~/.claude/skills</code>
          </p>
        </div>
      </div>
      ...
      <div v-else class="flex-1 overflow-y-auto skill-grid-bg">
        ...
        <div v-if="filteredSkills.length === 0 && searchQuery" class="flex items-center justify-center" style="padding:60px 32px;">
          <div class="text-center">
            ...
            <p style="font-family:'Figtree',sans-serif; font-size:var(--fs-body); font-weight:600; color:#475569; margin:0 0 4px;">No skills match "{{ searchQuery }}"</p>
    ...
    <!-- ════════════════════════════════════════════════════════════════════════
         ...
         ════════════════════════════════════════════════════════════════════════ -->
    <template v-else>
      <!-- Header with back button — glassmorphism bar -->
        ...
        <div class="detail-sidebar">
          ...
          <div class="flex-1 overflow-y-auto py-1.5" style="scrollbar-width:thin;">
            <div v-if="fileTree.length === 0" class="px-4 py-8 text-center">
              <p style="font-family:'Noto Sans',sans-serif; font-size:var(--fs-body); color:#94A3B8;">No files found</p>
            </div>
            <SkillTreeNode
          ...
          <div v-if="!activeFilePath" class="flex-1 flex items-center justify-center">
            <div class="text-center">
              <div class="detail-empty-icon">
                <svg style="width:32px;height:32px;color:#94A3B8;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                  <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/>
                </svg>
              </div>
              <p style="font-family:'Noto Sans',sans-serif; font-size:var(--fs-body); color:#94A3B8; margin-top:12px;">Select a file to view its contents</p>
            </div>
          </div>
          ...
          <template v-else>
            ...
            <div class="detail-file-tab">
              <div class="detail-file-tab-inner">
                <svg style="width:14px;height:14px;color:#6366F1;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/>
                </svg>
            ...
            <div class="detail-content-scroll">
              <div v-if="loadingFile" style="color:#64748B; font-family:'Noto Sans',sans-serif; font-size:var(--fs-body);">Loading...</div>
              <div v-else-if="fileError" style="color:#dc2626; font-family:'Noto Sans',sans-serif; font-size:var(--fs-body);">{{ fileError }}</div>
              <!-- Markdown rendering -->
              <div
      ...
      children.push(
        h('div', {
          ...
          style: {
            ...
            background: isActive ? 'rgba(99,102,241,0.1)' : (hovered.value ? 'rgba(99,102,241,0.04)' : 'transparent'),
          ...
          isDir
            ...
            : h('svg', { style: 'width:14px;height:14px;flex-shrink:0;color:#94A3B8;', viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', 'stroke-width': '2' }, [
...
/* ── Grid background: vibrant mesh for glassmorphism visibility ────────── */
.skill-grid-bg {
  background:
    radial-gradient(ellipse at 15% 10%, rgba(99, 102, 241, 0.12) 0%, transparent 50%),
    radial-gradient(ellipse at 85% 20%, rgba(236, 72, 153, 0.08) 0%, transparent 45%),
...
.prose-skills li::marker { color: #6366F1; }
.prose-skills code {
  font-family: 'JetBrains Mono', 'Fira Code', monospace;
  font-size: 0.85em;
...
.prose-skills a { color: #6366F1; text-decoration: none; font-weight: 500; }
.prose-skills a:hover { text-decoration: underline; color: #4F46E5; }
.prose-skills hr { border: none; border-top: 1px solid rgba(226, 232, 240, 0.6); margin: 28px 0; }
.prose-skills table { border-collapse: collapse; margin: 0 0 18px; width: 100%; border-radius: 10px; overflow: hidden; }
.prose-skills th, .prose-skills td { border: 1px solid rgba(226, 232, 240, 0.6); padding: 9px 14px; text-align: left; }
.prose-skills th { background: rgba(99, 102, 241, 0.04); font-weight: 600; color: #1E293B; }
</style>
