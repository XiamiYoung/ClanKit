...
function _applyReplacement(targetText, newText) {
  ...
  let idx = isWholeFileEdit ? -1 : currentFullContent.indexOf(targetText)
  let matchLen = targetText ? targetText.length : 0

  // If strict match fails, retry with a whitespace-normalized regex that also
  // tolerates Markdown table separators ("|"). This recovers the common case
  // where the user selects from the rendered Markdown view (cells joined by
  // whitespace) but the underlying source uses pipe-delimited table syntax.
  // Only accept a fallback hit when EXACTLY ONE candidate exists in the source —
  // otherwise we could pick the wrong region.
  if (!isWholeFileEdit && idx < 0 && targetText && /\s/.test(targetText)) {
    const escaped = targetText
      .replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
      .replace(/(?:\\[\s\S]|\s)+/g, '[\\s|]+')
    try {
      const re = new RegExp(escaped, 'g')
      const matches = [...currentFullContent.matchAll(re)]
      if (matches.length === 1) {
        idx = matches[0].index
        matchLen = matches[0][0].length
      }
    } catch {
      // Fallback regex compile failed (e.g. exotic chars) — leave idx at -1.
    }
  }
  ...
  if (isMarkdown.value && !editMode.value && formattedEl.value) {
    const updated = isWholeFileEdit
      ? newText
      : currentFullContent.slice(0, idx) + newText + currentFullContent.slice(idx + matchLen)
  ...
  if (isTextLike.value) {
    ...
    if (isWholeFileEdit) {
      ...
      const updated = currentFullContent.slice(0, idx) + newText + currentFullContent.slice(idx + matchLen)
      store.updateContent(updated)
      editorContent.value = updated
  ...
  if (store.activeFile?.content !== undefined) {
    ...
    if (isWholeFileEdit) {
      ...
      const updated = content.slice(0, idx) + newText + content.slice(idx + matchLen)
      store.updateContent(updated)
      editorContent.value = updated
    }
    return true
...
const TreeNode = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    return () => {
      ...
      const showRowBg = isActive || isHovered || (dragOver.value && isDir)

      // Main row — outer row stays transparent; gradient + shadow are painted by an
      // absolutely-positioned inner layer (mirrors ChatTreeNodeView). This avoids the
      // 1px transparent-border rendering artifact that produced a thin vertical line
      // along the right edge of items in the prior implementation.
      children.push(
        h('div', {
          class: 'flex items-center gap-2 py-1.5 pr-2 cursor-pointer group relative',
          style: {
            paddingLeft: indent + 'px',
            color: isDark ? '#fff' : '#6B7280',
            isolation: 'isolate',
            margin: showRowBg ? '4px 8px' : '4px 0',
          ...
          // Background layer — paints gradient + shadow without polluting the row's
          // own box, so no border/clip artifacts can leak through.
          showRowBg ? h('div', {
            'aria-hidden': 'true',
            style: {
              position: 'absolute',
              ...
              boxShadow: isActive
                ...
                : (isHovered ? '0 2px 8px rgba(0,0,0,0.10)' : 'none'),
              outline: dragOver.value && isDir ? '1px dashed #007AFF' : 'none',
              outlineOffset: '-1px',
              pointerEvents: 'none',
              zIndex: '-1',
