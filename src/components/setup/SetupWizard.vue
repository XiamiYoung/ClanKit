        ...
        <template v-if="phase === 'steps'">
          ...
          <div class="sw-content">
            ...
            <template v-else-if="step === 4">
              <div class="sw-step-content">
                ...
                <template v-if="selectedProviderType === 'custom'">
                  <div class="sw-form-group">
                    <label class="sw-label">Provider Name</label>
                    <input v-model="customName" class="sw-input" placeholder="My Custom Provider" />
                  </div>
                  <div class="sw-form-group">
                    <label class="sw-label">Protocol</label>
                    <select v-model="customProtocol" class="sw-select">
                ...
                <template v-if="selectedProviderType === 'anthropic'">
                  ...
                  <div class="sw-form-group">
                    ...
                    <input v-model="anthropicOpus" class="sw-input font-mono" placeholder="e.g. claude-opus-4-7" />
                  </div>
                  <div class="sw-form-group">
          ...
          <div class="sw-footer">
            ...
            <div class="sw-footer-right">
              <AppButton v-if="step !== 2" variant="primary" size="modal" :disabled="!canProceed || aiGenerating || installingRecommended || isNavigating" :loading="aiGenerating || installingRecommended || isNavigating" @click="goNext">
                {{ t('common.next') }}
              </AppButton>
            </div>
          </div>
...
// Refresh builtin agents (Clank/DocMaster/Analyst) name+description+prompt+
// voice from i18n whenever the user changes language during the wizard.
// Without this, the user can see them in the OLD language behind the wizard
// modal — Clank's prompt was seeded in App.vue's loadAgents() before step 1,
// using whatever default config.language was at startup. This mirrors the
// final pass at finishTour() so toggling languages back and forth keeps the
// background visibly in sync.
watch(() => configStore.language, async (lang, oldLang) => {
  if (!props.visible) return
  if (!lang || lang === oldLang) return
  try {
    await agentsStore.applyWizardCapabilitiesToBuiltins({})
  } catch (err) {
    console.warn('[SetupWizard] builtins language refresh failed:', err?.message)
  }
})
...
const wizardProviderTypes = ['google', 'qwen', 'anthropic', 'openai_official', 'deepseek', 'openrouter', 'groq', 'glm', 'moonshot', 'doubao', 'ollama', 'mistral', 'xai', 'custom']
...
const customProtocol = ref('openai')
...
const showBaseUrl = computed(() => {
  ...
  return type === 'ollama' || type === 'openai' || type === 'custom' || type === 'anthropic' || !currentPreset.value?.defaultBaseURL
})
...
function ensureWizardProvider() {
  ...
  if (existingByType) {
    ...
    if (type === 'custom') {
      customName.value = existingByType.name || ''
      customProtocol.value = existingByType.settings?.protocol || 'openai'
    }
    return existingByType
  }
  const name = type === 'custom' ? (customName.value || 'Custom') : null
  const provider = configStore.addProvider(type, name)
  wizardProviderId.value = provider.id
  providerBaseUrl.value = provider.baseURL || ''
  if (type === 'custom') {
    provider.settings.protocol = customProtocol.value
  }
  return provider
}
...
function syncProviderFields() {
  ...
  if (type === 'custom') {
    updates.name = customName.value || 'Custom'
...
async function finishTour() {
  try {
    ...
    // Builtins were seeded on first launch BEFORE the user picked a provider
    // and BEFORE tools/skills/mcp/knowledge stores finished loading, so
    // Clank's provider/model are null and its capability arrays are empty.
    // Push the wizard-selected provider/model onto all three builtins, and
    // hand Clank the full capability set so it can use everything out of
    // the box. Per SSOT rule we only do this here, never on later startups.
    const utility = configStore.config.utilityModel || {}
    await agentsStore.applyWizardCapabilitiesToBuiltins({
      providerId: utility.provider || selectedProviderType.value || null,
      modelId:    utility.model    || (selectedProviderType.value === 'anthropic' ? anthropicSonnet.value : selectedModelId.value) || null,
    })
    const chatTitle = t('setupWizard.firstChatName')
    // Wizard's first chat lives inside a dedicated demo folder so the user
    // sees an organized starting layout rather than a flat chat list. The
    // folder is created with expanded=true (default in createFolder); when
    // the user lands on the chat, setActiveChat → _expandAncestorFolders
    // also keeps it open even on subsequent visits. Single agent (Clank
    // only) and productivity mode for stability — first-impression chat,
    // not a multi-agent stress test.
    const demoFolder = await chatsStore.createFolder(t('setupWizard.demoFolderName'), null, '🎓')
    const chat = await chatsStore.createChat(chatTitle, [BUILTIN_SYSTEM_AGENT_ID], demoFolder?.id || null, { userAgentId, mode: 'productivity' })
    ...
    setTimeout(() => {
      if (chatId) {
        // First-chat greeting — single agent, single tool call, ~100 chars
        // of model output. Optimized for fast wait + low failure rate. The
        // AI Doc nav floating indicator below is gated on the chat going
        // from isRunning=true → false, which fires after Clank's write
        // tool call finishes and the final text streams in.
        const greeting = lang === 'zh'
          ? '@Clank 先介绍一下你自己能做什么。然后用一个 markdown 表格介绍 叮咣AI 这个 app 能帮我做什么（建议列：模块、能做什么、适合场景）。完成后保存为「关于叮咣AI.md」到 AI 文档，最后告诉我文件创建的路径。'
          : '@Clank First, introduce yourself and share what you can do. Then use a markdown table to introduce what the ClanKit app can help me with (suggested columns: module, what it does, when to use it). Save it as about-clankit.md in AI Doc, and finally tell me the path where the file was created.'
  ...
  if (step.value >= 3) {
    ...
    if (existing) {
      ...
      if (type === 'custom') {
        ...
        customProtocol.value = existing.settings?.protocol || 'openai'
      }
