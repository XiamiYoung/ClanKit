    ...
    >
      <template #actions>
        ...
        <div class="gp-swap-wrap" ref="swapWrapEl">
          <button ref="swapBtnEl" class="gp-swap-btn" :class="{ active: showSwapMenu }" @click.stop="toggleSwapMenu" v-tooltip="t('chats.switchToAnotherChat')">
        ...
        <button class="gp-maximize-btn" @click.stop="$emit('maximize')" v-tooltip="t('chats.openInSingleView')">
          <svg style="width:14px;height:14px;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
async function onMentionSend(text) {
  ...
  // Guard: refuse to send if any referenced agent is deleted (group + user).
  // Mirrors useSendMessage so the grid-panel mention path enforces the same
  // policy — no silent fallback to default agent.
  {
    const refs = []
    if (targetChat.systemAgentId) refs.push(targetChat.systemAgentId)
    if (Array.isArray(targetChat.groupAgentIds)) refs.push(...targetChat.groupAgentIds)
    if (targetChat.userAgentId) refs.push(targetChat.userAgentId)
    if (refs.some(id => agentsStore.isAgentDeleted(id))) {
      await chatsStore.addMessage(chatId, {
        role: 'assistant',
        content: t('chats.cannotSendDeletedAgent'),
      })
      return
    }
  }
  ...
  if (isGroupChat.value && groupIds.length > 1) {
    ...
    try {
      for (const agentId of respondingIds) {
        ...
        singleCfg.modeTransitionPending = targetChat.modeTransitionPending ? JSON.parse(JSON.stringify(targetChat.modeTransitionPending)) : null
        ...
        try {
          ...
          if (currentChat?.messages) {
            const msg = currentChat.messages.find(m => m.id === streamingMsgId)
            if (msg) { msg.content = `Error: ${err.message}`; msg.streaming = false; if (msg.streamingStartedAt) msg.durationMs = Date.now() - msg.streamingStartedAt; msg.timestamp = Date.now() }
          }
        }
      }
    } finally {
      ...
      if (finChat?.messages) for (const m of finChat.messages) if (m.streaming) { m.streaming = false; if (m.streamingStartedAt) m.durationMs = Date.now() - m.streamingStartedAt; m.timestamp = Date.now() }
...
async function onSend(text, pendingAttachments = [], longBlobs = {}) {
  ...
  // Guard: refuse to send when chat references a deleted agent.
  {
    const refs = []
    if (targetChat.systemAgentId) refs.push(targetChat.systemAgentId)
    if (Array.isArray(targetChat.groupAgentIds)) refs.push(...targetChat.groupAgentIds)
  ...
  cfg.modeTransitionPending = targetChat.modeTransitionPending ? JSON.parse(JSON.stringify(targetChat.modeTransitionPending)) : null
  ...
  try {
    ...
    if (targetChat.messages) {
      ...
      if (msg) {
        ...
        msg.streaming = false; if (msg.streamingStartedAt) msg.durationMs = Date.now() - msg.streamingStartedAt; msg.timestamp = Date.now()
      }
    }
    await chatsStore.persist?.()
  } catch (err) {
    const tc = chatsStore.chats.find(c => c.id === chatId)
    if (tc?.messages) { const msg = tc.messages.find(m => m.id === streamingMsgId); if (msg) { msg.content = `Error: ${err.message}`; msg.streaming = false; if (msg.streamingStartedAt) msg.durationMs = Date.now() - msg.streamingStartedAt; msg.timestamp = Date.now() } }
  } finally {
    const finChat = chatsStore.chats.find(c => c.id === chatId)
    if (finChat) { finChat.isRunning = false; finChat.isThinking = false }
    if (finChat?.messages) for (const m of finChat.messages) if (m.streaming) { m.streaming = false; if (m.streamingStartedAt) m.durationMs = Date.now() - m.streamingStartedAt; m.timestamp = Date.now() }
