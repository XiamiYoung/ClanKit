<template>
  <Teleport to="body">
    ...
    >
      ...
      <!-- Last error from the active call (LLM 4xx, STT failure, etc.) -->
      <div v-if="voiceStore.lastError" class="call-panel-error" @click="voiceStore.setError('')" :title="t('common.close')">
        <span class="call-panel-error-label">{{ t('voice.errorLabel') }}</span>
        <span class="call-panel-error-text">{{ voiceStore.lastError }}</span>
      </div>
...
/* Error block — same shape as transcript, red accent. Click to dismiss. */
.call-panel-error {
  margin: 0 0.625rem 0.375rem;
  padding: 0.375rem 0.625rem;
